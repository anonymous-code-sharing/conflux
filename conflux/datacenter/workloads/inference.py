...
from conflux.datacenter.config import InferenceModelSpec
...
class InferenceTraceStore:
    ...
    def build_templates(
        ...
        *,
        ...
        Returns:
            A [`InferenceTemplateStore`][conflux.datacenter.workloads.inference.InferenceTemplateStore]
...
def _check_version_stamp(data_dir: Path, label: str) -> None:
    """Log a warning if cached data was generated with a different conflux version."""
    ...
    try:
        ...
        return
    cached_version = manifest.get("conflux_version", "unknown")
    if cached_version != conflux.__version__:
        logger.warning(
            "%s: cached data generated with conflux %s (current %s). Consider regenerating.",
            label,
            cached_version,
            conflux.__version__,
        )
...
def _write_inference_manifest(
    ...
    Each file records the full spec fields, conflux/mlenergy-data
    ...
    for ms in specs:
        ...
        manifest = {
            ...
            "spec": spec_fields,
            "conflux_version": conflux.__version__,
