...
class InferenceModelSpec(BaseModel):
    ...
    *how many* or *at what batch size*.  Deployment-specific parameters
    ...
    [`ModelDeployment`][conflux.datacenter.config.ModelDeployment].
...
class ModelDeployment:
    ...
    Pairs an [`InferenceModelSpec`][conflux.datacenter.config.InferenceModelSpec] (model identity)
    with the initial batch size. Replica counts (including runtime ramps)
    live on [`ReplicaSchedule`][conflux.datacenter.config.ReplicaSchedule]
...
class TrainingRun:
    ...
    Attributes:
        ...
        trace: Single-GPU [`TrainingTrace`][conflux.datacenter.workloads.training.TrainingTrace].
