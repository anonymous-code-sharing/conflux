...
class LogisticModelStore:
    ...
    Used by
    [`OFOBatchSizeController`][conflux.controller.ofo.OFOBatchSizeController]
...
class VoltageDualVariables:
    ...
    def __init__(self, n_bus_phases: int, config: OFOConfig) -> None:
        self.config = config
        self.dual_undervoltage = np.zeros(int(n_bus_phases), dtype=float)  # λ in original paper Eq. 5
        self.dual_overvoltage = np.zeros(int(n_bus_phases), dtype=float)  # λ̄ in original paper Eq. 6
    ...
    def step(
        ...
        *,
        ...
        for ms in self.models:
            ...
            # original paper Eq. 18: nabla_x L = -dR/dx (throughput)
...
class OFOBatchSizeController(Controller[LLMBatchSizeControlledDatacenter[LLMDatacenterState], OpenDSSGrid]):
    ...
    ][conflux.datacenter.base.LLMDatacenterState.observed_itl_s_by_model].
