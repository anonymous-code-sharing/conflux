...
Walks the conflux package tree, groups modules by top-level package,
...
PACKAGE_DIR = Path(__file__).resolve().parent.parent / "conflux"
...
def discover_modules() -> dict[str, list[str]]:
    ...
    Top-level modules (e.g. `conflux.clock`) map to a single-element list.
    Packages (e.g. `conflux.datacenter`) map to all their submodules.
    """
    ...
    for py_file in sorted(PACKAGE_DIR.rglob("*.py")):
        ...
        parts = module.split(".")  # e.g. ["conflux", "datacenter", "offline"]
        page_name = parts[1]
