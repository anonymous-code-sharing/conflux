...
from conflux.coordinator import SimulationLog
from conflux.datacenter.config import DatacenterConfig, InferenceModelSpec, PowerAugmentationConfig
from conflux.datacenter.layout import ServerPool
from conflux.datacenter.online import (
    GPUEndpointMapping,
    LiveServerConfig,
    OnlineDatacenter,
    VLLMDeployment,
...
from conflux.datacenter.workloads.inference import InferencePowerAugmenter
...
def _make_itl_stub(model_label: str, itl_window_s: float = 1.0):
    ...
    from conflux.datacenter.online import _LoadGenerator
...
def _online_dc(
    ...
    with (
        patch("conflux.datacenter.online._LoadGenerator"),
        patch("conflux.datacenter.online.PowerStreamingClient", return_value=fake_client),
        patch("conflux.datacenter.online.STAGGER_BUFFER_S", stagger_buffer_s),
    ):
        dc = OnlineDatacenter(
...
class TestOnlineAugmentationPipeline:
    ...
    def _build_pool_and_augmenter(
        ...
        from conflux.utils import split_integer_evenly
...
class TestHealthChecks:
    def test_check_vllm_health_failure(self) -> None:
        from conflux.datacenter.online import _check_vllm_health
    ...
    def test_check_vllm_model_failure(self) -> None:
        from conflux.datacenter.online import _check_vllm_model
    ...
    def test_check_zeusd_health_failure(self) -> None:
        from conflux.datacenter.online import _check_zeusd_health
