...
from conflux.controller.ofo import LogisticModelStore, OFOConfig
from conflux.controller.rule_based import RuleBasedBatchSizeController, RuleBasedConfig
from conflux.controller.tap_schedule import TapScheduleController
from conflux.coordinator import Coordinator
from conflux.datacenter.config import DatacenterConfig, InferenceModelSpec, ReplicaSchedule, TrainingRun
...
from conflux.grid.config import TapSchedule
from conflux.metrics.voltage import VoltageStats, compute_allbus_voltage_stats
...
def run_simulation(
    ...
    *,
    ...
    if mode == "ofo":
        from conflux.controller.ofo import OFOBatchSizeController
        ...
        from conflux.controller.ppo import PPOBatchSizeController, SharedPPOBatchSizeController
        from conflux.rl.env import ObservationConfig
        ...
        if shared_model is not None and shared_model.exists():
            ...
            if obs_mode == "per-zone-summary":
                ...
                from conflux.rl.env import compute_bus_phase_groups
    ...
    from conflux.controller.ppo import SharedPPOBatchSizeController
...
class ScenarioRecord:
    ...
    in any other process (e.g. train_ppo.py). See conflux/rl/env.py:ScenarioLibrary
    stub registration.
    """
    ...
    # need to replay randomize_scenario.
...
def _per_step_voltage_pen(grid_states, *, v_min: float, v_max: float, exclude_buses: tuple[str, ...]) -> np.ndarray:
    ...
    This matches ``compute_reward`` in ``conflux/rl/env.py``: at each step the
...
def main(
    *,
    ...
    logging.getLogger("conflux.coordinator").setLevel(logging.WARNING)
    logging.getLogger("conflux.datacenter").setLevel(logging.WARNING)
    logging.getLogger("conflux.grid").setLevel(logging.WARNING)
    logging.getLogger("conflux.controller.ofo").setLevel(logging.WARNING)
