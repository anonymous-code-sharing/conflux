...
def make_sim_factory(
    ...
    each ``ScenarioRecord``; the rng-replay path is gone. Old libraries are
    unsupported; rebuild with ``build_scenario_library.py``.
    """
    sys = exp["sys"]
...
def main() -> None:
    ...
    from conflux.controller.ofo import LogisticModelStore
    ...
    if args.scenario_library:
        from conflux.rl.env import ScenarioLibrary
