...
from conflux.controller.ofo import OFOConfig
...
class ScenarioOpenDSSGrid(OpenDSSGrid):
    ...
    def _init_dss(self) -> None:
        super()._init_dss()
        from conflux.grid.opendss import dss
...
def materialize_scenario(rec, *, training_base: dict | None) -> dict:
    ...
    libraries built before this field set are unsupported and must be
    rebuilt with the current ``build_scenario_library.py``.
    """
    if getattr(rec, "resolved_dc_sites", None) is None:
        raise RuntimeError(
            f"ScenarioRecord(seed={rec.seed}) is missing resolved fields. "
            "Rebuild the library with the current build_scenario_library.py."
        )
