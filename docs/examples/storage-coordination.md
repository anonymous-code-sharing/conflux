...
All four storage systems use a 2-hour duration and initial SOC of 0.5. The example leaves [`BatteryStorage.idle_loss_percent`][conflux.grid.storage.BatteryStorage] at its default value of `0.0`, so idle and pure reactive operation do not intentionally drain stored energy.

The controlled storage scenarios use [`LocalVoltageStorageDroopController`][conflux.controller.storage.LocalVoltageStorageDroopController] with [`StorageDroopConfig`][conflux.controller.storage.StorageDroopConfig]:
