...
Conflux supports **online mode** where the datacenter backend reads live GPU power from running vLLM inference servers via [Zeus](https://ml.energy/zeus), while the grid backend still runs OpenDSS power flow in simulation. This enables hardware-in-the-loop (HIL) experiments that validate whether control algorithms developed in offline simulation transfer to real GPU hardware.
