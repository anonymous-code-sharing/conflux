...
This page walks through building your own simulator for your scenario using Conflux components.
...
You wire them together with the [`Coordinator`][conflux.coordinator.Coordinator]:
...
The [`OfflineDatacenter`][conflux.datacenter.offline.OfflineDatacenter] replays GPU power traces built from the [data pipeline](data-pipeline.md).
Load traces from the generated manifest, build templates for the simulation config, then create the datacenter with a [`DatacenterConfig`][conflux.datacenter.config.DatacenterConfig] and [`OfflineWorkload`][conflux.datacenter.offline.OfflineWorkload]:

```python
from fractions import Fraction
from pathlib import Path

from conflux.datacenter.config import (
...
from conflux.datacenter.workloads.training import TrainingTrace
...
[`ReplicaSchedule`][conflux.datacenter.config.ReplicaSchedule] specifies each model's replica count over time. The initial count and optional ramps are defined in one object using method chaining: `ReplicaSchedule(initial=720).ramp_to(360, t_start=1500, t_end=2000)`. The datacenter validates that ramp targets do not exceed `total_gpu_capacity` before running.
...
Construct an [`OpenDSSGrid`][conflux.grid.opendss.OpenDSSGrid] with a DSS case directory, then attach dynamic components to specific buses. The DSS file defines the base network (lines, transformers, regulators, static loads). Attached components add dynamic power sources and sinks on top -- their power is updated each timestep during simulation.

The grid looks up bus voltage levels from the DSS model automatically, so you never need to specify `bus_kv`. See [`OpenDSSGrid`][conflux.grid.opendss.OpenDSSGrid] for additional options (`source_pu`, `exclude_buses`, etc.).
...
from conflux.grid.generator import SyntheticPV
from conflux.grid.load import SyntheticLoad
from conflux.grid.storage import BatteryStorage
...
All `attach_*` calls must happen before `start()` (which the [`Coordinator`][conflux.coordinator.Coordinator] calls automatically). Static loads defined in the DSS file coexist with attached dynamic components -- the power flow sees both.

#### Generators, loads, and storage

[`Generator`][conflux.grid.generator.Generator] and [`ExternalLoad`][conflux.grid.load.ExternalLoad] are abstract base classes with a single `power_kw(t)` method. [`EnergyStorage`][conflux.grid.storage.EnergyStorage] is similar but bidirectional and stateful: it returns both `power_kw(t)` and `reactive_power_kvar(t)`, and accepts external setpoints via `set_power_kw(...)`. Built-in implementations:

| Class | Kind | Description |
|-------|------|-------------|
| [`SyntheticPV`][conflux.grid.generator.SyntheticPV] | Generator | Demonstration PV profile with cloud dips and trends |
| [`ConstantGenerator`][conflux.grid.generator.ConstantGenerator] | Generator | Fixed power output |
| [`CSVProfileGenerator`][conflux.grid.generator.CSVProfileGenerator] | Generator | Interpolated from a CSV time series |
| [`SyntheticLoad`][conflux.grid.load.SyntheticLoad] | External load | Demonstration load with diurnal bumps |
| [`ConstantLoad`][conflux.grid.load.ConstantLoad] | External load | Fixed power consumption |
| [`CSVProfileLoad`][conflux.grid.load.CSVProfileLoad] | External load | Interpolated from a CSV time series |
| [`BatteryStorage`][conflux.grid.storage.BatteryStorage] | Storage | Mutable setpoint battery backed by native OpenDSS `Storage` physics |
...
Storage is the only one of the three that the simulation can actively command at runtime: controllers emit [`SetStoragePower`][conflux.grid.command.SetStoragePower] commands carrying the storage object, and Conflux ships a [`LocalVoltageStorageDroopController`][conflux.controller.storage.LocalVoltageStorageDroopController] for local Q-V or P-V droop. See [`BatteryStorage`](#batterystorage) and [`LocalVoltageStorageDroopController`](#localvoltagestoragedroopcontroller) below.

#### Tap schedules

Tap *schedules* are built using [`TapPosition`][conflux.grid.config.TapPosition] and the `|` operator, and passed to a [`TapScheduleController`][conflux.controller.tap_schedule.TapScheduleController]:
...
Conflux ships with IEEE test feeder models in `data/grid/`, one folder per system. Each folder contains a self-contained master `.dss` file and a shared line impedance file:
...
```python
from conflux.controller.tap_schedule import TapScheduleController
from conflux.controller.ofo import LogisticModelStore, OFOBatchSizeController, OFOConfig
...
[`Coordinator.run()`][conflux.coordinator.Coordinator.run] returns a [`SimulationLog`][conflux.coordinator.SimulationLog] containing all state and action history:

```python
log = coord.run()

# Voltage statistics (grid-side quality)
from conflux.metrics.voltage import compute_allbus_voltage_stats
...
from conflux.metrics.performance import compute_performance_stats
...
See [`examples/offline/plotting.py`](https://github.com/anonymous-code-sharing/conflux/blob/master/examples/offline/plotting.py) for reusable plot functions used by the example scripts.

## Writing Custom Components

Conflux is designed for extensibility.
...
**Datacenter backend.**
Subclass [`DatacenterBackend`][conflux.datacenter.base.DatacenterBackend] and parameterize it with the state type your backend returns from [`step()`][conflux.datacenter.base.DatacenterBackend.step]. Your `__init__` must call `super().__init__()` to initialize the base class's state tracking. The coordinator calls [`do_step()`][conflux.datacenter.base.DatacenterBackend.do_step] at the rate specified by [`dt_s`][conflux.datacenter.base.DatacenterBackend.dt_s], which internally calls your `step()` and records the returned state. Each `step()` call should return a [`DatacenterState`][conflux.datacenter.base.DatacenterState] (or subclass) with three-phase power in watts. Between steps, the coordinator may call [`apply_control(command, events)`][conflux.datacenter.base.DatacenterBackend.apply_control] with individual [`DatacenterCommand`][conflux.datacenter.command.DatacenterCommand] instances; changes take effect on the next `step()`. Current state is available through [`state`][conflux.datacenter.base.DatacenterBackend.state] and past states through [`history(n)`][conflux.datacenter.base.DatacenterBackend.history]; both are managed automatically by the base class.

[`reset()`][conflux.datacenter.base.DatacenterBackend.reset] must clear all simulation state (counters, RNG state) while preserving configuration (`dt_s`, models, templates). History is cleared automatically by the coordinator (via [`do_reset()`][conflux.datacenter.base.DatacenterBackend.do_reset]). Override [`start()`][conflux.datacenter.base.DatacenterBackend.start] and [`stop()`][conflux.datacenter.base.DatacenterBackend.stop] for backends that acquire resources (connections, solver circuits); the coordinator calls `start()` before the simulation loop and `stop()` in LIFO order afterward. See [Architecture: Component Lifecycle](architecture.md#component-lifecycle) for the full sequence.
...
from conflux.common import ThreePhase
from conflux.datacenter.base import DatacenterState
from conflux.datacenter.command import DatacenterCommand, SetBatchSize
...
**Datacenter state.**
For richer state, define a [`DatacenterState`][conflux.datacenter.base.DatacenterState] subclass:
...
The state type propagates through to [`SimulationLog`][conflux.coordinator.SimulationLog], so `log.dc_states` will be correctly typed as `list[MyState]`.

**Datacenter commands.**
For custom commands, subclass [`DatacenterCommand`][conflux.datacenter.command.DatacenterCommand] and register a handler with `@apply_control.register`:
...
The coordinator routes commands to backends based on their type hierarchy: [`DatacenterCommand`][conflux.datacenter.command.DatacenterCommand] subclasses go to the datacenter, [`GridCommand`][conflux.grid.command.GridCommand] subclasses go to the grid.

**Testing.** [`DatacenterState`][conflux.datacenter.base.DatacenterState] and [`GridState`][conflux.grid.base.GridState] are simple frozen dataclasses, so you can construct them directly in unit tests without running a full simulation.

### Custom Controller

**Controller.**
Subclass [`Controller`][conflux.controller.base.Controller] and parameterize it with the datacenter and grid backend types it works with. Bind the datacenter(s) and grid at construction time. The coordinator calls [`step(clock, events)`][conflux.controller.base.Controller.step] at the rate specified by [`dt_s`][conflux.controller.base.Controller.dt_s]; each call must return a list of [`DatacenterCommand`][conflux.datacenter.command.DatacenterCommand] and/or [`GridCommand`][conflux.grid.command.GridCommand] objects (return `[]` for a no-op). Read current state via the stored datacenter and grid references (e.g., `self._datacenter.state`, `self._grid.state`). Use [`events.emit(topic, data)`][conflux.events.EventEmitter.emit] to log controller-side events. Keep `step()` fast since it runs synchronously in the simulation loop. All datacenter commands must set `target` to the datacenter they apply to.

[`reset()`][conflux.controller.base.Controller.reset] must clear all simulation state (e.g., dual variables, counters, cached matrices) while preserving configuration. Override [`start()`][conflux.controller.base.Controller.start] and [`stop()`][conflux.controller.base.Controller.stop] if the controller acquires external resources. See [Architecture: Component Lifecycle](architecture.md#component-lifecycle) for the full sequence.
...
Controllers that need specific backend features (e.g., `voltages_vector()` from [`OpenDSSGrid`][conflux.grid.opendss.OpenDSSGrid]) should bind their generic type parameters to concrete types instead of using the base classes. See the [OFO controller](#ofobatchsizecontroller) below for an example.
...
[`OfflineDatacenter`][conflux.datacenter.offline.OfflineDatacenter] implements [`DatacenterBackend`][conflux.datacenter.base.DatacenterBackend] by replaying real GPU power traces at the controller's commanded batch size. Each step it allocates servers per model from a shared phase-balanced [`ServerPool`][conflux.datacenter.layout.ServerPool], samples per-GPU power templates with per-server stagger and noise, overlays any active training workload, and sums into per-phase totals.
...
[`SetBatchSize`][conflux.datacenter.command.SetBatchSize] takes effect on the next step; [`ShiftReplicas`][conflux.datacenter.command.ShiftReplicas] adjusts a per-model replica offset and the caller is responsible for checking GPU capacity.

### `OpenDSSGrid`

[`OpenDSSGrid`][conflux.grid.opendss.OpenDSSGrid] implements [`GridBackend`][conflux.grid.base.GridBackend] by wrapping a compiled OpenDSS circuit and running one power-flow solve per step. `start()` compiles the user's DSS file, builds the bus-phase voltage index, and adds native `Storage` elements for any attached storage (storage requires a three-phase bus). Each step writes datacenter, generator, external-load, and storage setpoints into OpenDSS, solves, and returns per-bus, per-phase voltages plus tap positions; each attached storage then receives a [`StorageState`][conflux.grid.storage.StorageState] readback through `update_state()`. The grid accepts [`SetTaps`][conflux.grid.command.SetTaps] and [`SetStoragePower`][conflux.grid.command.SetStoragePower], and exposes `voltages_vector()` and a finite-difference `estimate_sensitivity()` (dV/dP) for controllers that need fine-grained voltage information — used by [`OFOBatchSizeController`](#ofobatchsizecontroller).

### `BatteryStorage`

[`BatteryStorage`][conflux.grid.storage.BatteryStorage] implements [`EnergyStorage`][conflux.grid.storage.EnergyStorage] as a mutable setpoint battery. `set_power_kw(...)` validates real and apparent power against the rating and stores the request; `OpenDSSGrid` reads the setpoint each step, writes it to a native OpenDSS `Storage` element, and after the solve calls `update_state()` with the OpenDSS-reported [`StorageState`][conflux.grid.storage.StorageState] (cached on `storage.state`). `BatteryStorage.sized_for_datacenter(...)` is a convenience constructor that derives ratings from a datacenter's real power (default 20% rated power, 2-hour duration).

### `OFOBatchSizeController`

[`OFOBatchSizeController`][conflux.controller.ofo.OFOBatchSizeController] implements [`Controller`][conflux.controller.base.Controller] using primal-dual Online Feedback Optimization. It illustrates the typical tightly-coupled controller: it binds to a specific datacenter and grid via its generic type parameters, each step reads observed ITL and replica counts from the bound datacenter and `voltages_vector()` + `estimate_sensitivity()` from the bound grid, and emits one [`SetBatchSize`][conflux.datacenter.command.SetBatchSize] per model with `target` set. In multi-DC setups, use one controller per datacenter so each computes sensitivities only for its own DC's loads. See the original paper for the math.

### `LoadShiftController`

[`LoadShiftController`][conflux.controller.load_shift.LoadShiftController] implements [`Controller`][conflux.controller.base.Controller] for cross-site coordination. It holds references to all datacenters and, in a single step, emits paired [`ShiftReplicas`][conflux.datacenter.command.ShiftReplicas] commands targeting different DCs (`-` at the source, `+` at the destination) — illustrating a controller that spans multiple datacenters and produces a coordinated multi-target action. It also has an ordering relationship with other controllers: its activation rule depends on whether per-site batch controllers are saturated, so it must run after them.

### `LocalVoltageStorageDroopController`

[`LocalVoltageStorageDroopController`][conflux.controller.storage.LocalVoltageStorageDroopController] implements [`Controller`][conflux.controller.base.Controller] for storage rather than datacenters. It illustrates two patterns the other built-in controllers don't: targeting an [`EnergyStorage`][conflux.grid.storage.EnergyStorage] (via [`SetStoragePower`][conflux.grid.command.SetStoragePower]), and reading windowed grid *history* rather than just the current state — each step it consumes the history accumulated since the previous tick, reduces each storage's local samples by the configured `voltage_statistic`, and runs a deadbanded droop curve clipped to the storage rating. In Q-V mode the output drives kvar; in P-V mode it drives kW.
