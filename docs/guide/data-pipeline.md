...
Conflux ships with built-in support for trace-replay simulations based on [real GPU benchmark data](https://ml.energy/data).
...
|---|---|---|
| [`InferenceData`][conflux.datacenter.workloads.inference.InferenceData] | Power traces + ITL distribution fits | [`OfflineDatacenter`][conflux.datacenter.offline.OfflineDatacenter] |
| [`LogisticModelStore`][conflux.controller.ofo.LogisticModelStore] | Logistic curve fits (power, latency, throughput) | [`OFOBatchSizeController`][conflux.controller.ofo.OFOBatchSizeController] |
| [`TrainingTrace`][conflux.datacenter.workloads.training.TrainingTrace] | Synthetic training power trace | [`OfflineDatacenter`][conflux.datacenter.offline.OfflineDatacenter] |
...
Benchmark selection now lives directly on [`InferenceModelSpec`][conflux.datacenter.config.InferenceModelSpec]. A spec includes the benchmark query fields (`model_id`, `gpu_model`, `task`, `gpus_per_replica`, `batch_sizes`) together with serving-time knobs (`itl_deadline_s`, `feasible_batch_sizes`, `fit_exclude_batch_sizes`).
...
[`InferenceData.generate()`][conflux.datacenter.workloads.inference.InferenceData.generate] uses the [`mlenergy-data`](https://ml.energy/data) toolkit to download and process GPU benchmark data from the [ML.ENERGY Benchmark v3 dataset](https://huggingface.co/datasets/ml-energy/benchmark-v3).
...
[`LogisticModelStore.generate()`][conflux.controller.ofo.LogisticModelStore.generate] fits four-parameter logistic curves to power, latency, and throughput versus batch size:
...
Conflux uses [`LogisticModel`][mlenergy_data.modeling.LogisticModel] from [`mlenergy-data`](https://ml.energy/data) at both stages:
...
Conflux uses [`ITLMixtureModel`][mlenergy_data.modeling.ITLMixtureModel] from [`mlenergy-data`](https://ml.energy/data) at both stages:
...
[`TrainingTrace.generate()`][conflux.datacenter.workloads.training.TrainingTrace.generate] synthesizes a training power trace with configurable high/low plateaus, noise, brief dips, and a warm-up ramp. Generation is based on characteristics derived from real large model training measurements.

Parameters are controlled via [`TrainingTraceParams`][conflux.datacenter.workloads.training.TrainingTraceParams]. The empty dict `{}` in the config uses all defaults.
...
- **[`OfflineDatacenter`][conflux.datacenter.offline.OfflineDatacenter]**: Uses [`InferenceData`][conflux.datacenter.workloads.inference.InferenceData] to replay periodic per-GPU power templates. Latency fits ([`ITLMixtureModel`][mlenergy_data.modeling.ITLMixtureModel]) are sampled at each control interval.
- **[`OFOBatchSizeController`][conflux.controller.ofo.OFOBatchSizeController]**: Uses [`LogisticModelStore`][conflux.controller.ofo.LogisticModelStore] for logistic curve evaluation. Calls `eval()` and `deriv_wrt_x()` at each control step to compute gradients.
