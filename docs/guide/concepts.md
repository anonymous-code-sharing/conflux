...
This page introduces the problem space that Conflux addresses and the key concepts behind the library. For the original research motivation, see the original paper.
...
That's where Conflux comes in.
Conflux is a modular library that provides abstractions for simulating and developing these strategies for datacenter-grid coordination.
...
Conflux ships with several built-in controllers including an [Online Feedback Optimization (OFO)](building-simulators.md#ofobatchsizecontroller) controller for batch size regulation, a tap schedule controller, a local-voltage-based storage droop controller, and a no-op controller for baselines. You can also implement your own by subclassing the [`Controller`][conflux.controller.base.Controller] interface; see [Writing Custom Components](building-simulators.md#writing-custom-components).

## What Can You Explore?

Conflux is designed for researchers studying questions like:
